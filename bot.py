...
Fixed VirusTotal API integration - Working version
"""
...
async def search_ip(ip: str) -> Set[str]:
    """Search domains using all sources including VirusTotal"""
    all_domains = set()
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in futures:
            try:
                ...
                logger.error(f"Search error in thread: {str(e)}")
...
def query_yougetsignal(ip: str) -> List[str]:
    ...
    try:
        ...
        logger.info(f"Found {len(domains)} domains from source 1 for {ip}")
    except Exception as e:
...
def query_hackertarget(ip: str) -> List[str]:
    ...
    try:
        ...
        logger.info(f"Found {len(domains)} domains from source 2 for {ip}")
    except Exception as e:
...
def query_viewdns(ip: str) -> List[str]:
    ...
    try:
        ...
        if 'Reverse IP Lookup Results' in resp.text:
            ...
            for line in lines:
                if any(ext in line for ext in ['.com', '.net', '.org', '.io', '.xyz', '.info', '.biz']):
        ...
        logger.info(f"Found {len(domains)} domains from source 3 for {ip}")
    except Exception as e:
...
def query_criminalip(ip: str) -> List[str]:
    ...
    try:
        ...
        logger.info(f"Found {len(domains)} domains from source 4 for {ip}")
    except Exception as e:
        logger.error(f"Source 4 Error: {str(e)}")
    return domains

def query_virustotal(ip: str) -> List[str]:
    """Source 5: VirusTotal API"""
    domains = []
    
    try:
        logger.info(f"VirusTotal: Starting search for IP {ip}")
        ...
        if response.status_code == 200:
            ...
            if 'data' in data and 'attributes' in data['data']:
                ...
                if 'resolutions' in attributes and attributes['resolutions']:
                ...
                if 'last_resolution' in attributes and attributes['last_resolution']:
                    if 'hostname' in attributes['last_resolution']:
                        hostname = attributes['last_resolution']['hostname']
                        if hostname and '.' in hostname:
                            domains.append(hostname.lower())
            
            domains = list(set(domains))
            logger.info(f"Found {len(domains)} domains from source 5 for {ip}")
        
        # Backup using v2 API
        try:
            url_v2 = "https://www.virustotal.com/vtapi/v2/ip-address/report"
            params_v2 = {
                "apikey": VIRUSTOTAL_API_KEY,
                "ip": ip
            }
            response_v2 = requests.get(url_v2, params=params_v2, timeout=20)
            
            if response_v2.status_code == 200:
                data_v2 = response_v2.json()
                if 'resolutions' in data_v2:
                    for resolution in data_v2['resolutions']:
                        if 'hostname' in resolution:
                            hostname = resolution['hostname']
                            if hostname and '.' in hostname:
                                domains.append(hostname.lower())
                domains = list(set(domains))
                logger.info(f"Found additional {len(domains)} domains from source 5 backup")
                
        except Exception as e:
            logger.error(f"Source 5 backup error: {str(e)}")
        ...
        for domain in domains:
            if (len(domain) < 100 and 
                ...
                not domain.endswith('.arpa')):
                valid_domains.append(domain)
        
        if valid_domains:
            logger.info(f"VirusTotal found {len(valid_domains)} valid domains for {ip}")
